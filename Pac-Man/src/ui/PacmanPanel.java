...
/**
 * UI for a Pacman game. This UI does very little besides a) send periodic
 * update commands to the game, b) send turn instructions to the game when one
 * of the arrow keys is pressed, and c) draw the game state
 */
		...
		for (int row = 0; row < game.getNumRows(); ++row) {
			for (int col = 0; col < game.getNumColumns(); ++col) {
				...
				if (c.isWall()) {
					...
					g.drawRect(col * CELL_SIZE + BORDER_SIZE / 2, row * CELL_SIZE + BORDER_SIZE / 2, BORDER_SIZE,
							BORDER_SIZE);
				} else {
					if (c.getType() == CellType.DOT && c.canEat()) {
						g.setColor(Color.WHITE);
						g.fillOval(col * CELL_SIZE + (CELL_SIZE - PELLET_SIZE) / 2,
								row * CELL_SIZE + (CELL_SIZE - PELLET_SIZE) / 2, PELLET_SIZE, PELLET_SIZE);
					} else if (c.getType() == CellType.ENERGIZER && c.canEat()) {
						g.setColor(Color.LIGHT_GRAY);
						// g.fillOval(col * cellSize + (cellSize - PELLET_SIZE) / 2, row * cellSize +
						// (cellSize - PELLET_SIZE) / 2, PELLET_SIZE, PELLET_SIZE);
						g.fillOval(col * CELL_SIZE + BORDER_SIZE / 2, row * CELL_SIZE + BORDER_SIZE / 2, BORDER_SIZE,
								BORDER_SIZE);
