...
public class Pacman extends Character {
	...
	public void tryTurn(Direction newDir) {
		...
		if (diff >= 0 && diff < tolerance && !super.getMaze().isWall(newRowNum, newColNum)) {
	...
	@Override
	/**
	 * Updates Pacman's position. This method is typically invoked once per
	 * frame.
	 * 
	...
	public void update(Descriptor d) {
		...
		// negative
		double diff = distanceToCenter(super.getColExact(), super.getRowExact(), super.getCurrentDirection());
		...
		case LEFT:
			...
			if (curColExact - increment - 0.5 < 0) {
				curColExact = super.getMaze().getNumColumns() + (curColExact - increment - 0.5);
			} else {
				...
				// increment
				if (diff > -ERR && diff < increment && super.getMaze().isWall(rowNum, colNum - 1)) {
					increment = diff;
				}
		...
		case RIGHT:
			// special case: check whether we are in the tunnel and need to wrap around
			if (curColExact + increment + 0.5 >= super.getMaze().getNumColumns()) {
				curColExact = curColExact + increment + 0.5 - super.getMaze().getNumColumns();
			} else {
				if (diff > -ERR && diff < increment && super.getMaze().isWall(rowNum, colNum + 1)) {
					increment = diff;
				}
			...
			break;
		case UP:
			if (diff > -ERR && diff < increment && super.getMaze().isWall(rowNum - 1, colNum)) {
				increment = diff;
			}
	...
	@Override
	/**
	 * Does nothing in Pacman
	 * 
	 * @param mode	New mode to be set.
	 * @param desc	Descriptor of the current game.
	 */
	 ...
	 * Does nothing in Pacman
	 * 
	 * @return	null for pacman
	 */
	protected Location getNextCell() {
		// Do Nothing
		return null;
	}
